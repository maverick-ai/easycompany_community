    ...
    <div className={styles.profileContainer}>
      <div>
        <div className="user-stats">
          <div className="row">
            <div
              className={`col-6 ${styles.profileCol} ${styles.statsContainer}`}
            >
              <div>
                ...
                <p className={styles.repText}>
                  Reputation:{" "}
                  {userData.stats.total_score ? userData.stats.total_score : 0}
                </p>
              </div>
            </div>
          ...
          <div className={styles.colCard}>
            ...
            {userData.stats.badges &&
              userData.stats.badges.map((badge) => {
          ...
          <div className={`col-md-6 ${styles.profileCol}`}>
            <div className={styles.colCard}>
              <Link
                href={
                  query.user
                    ? `user/posts?user=${query.user}&page=1`
                    : `user/posts?page=1`
                }
              >
                <a>
              ...
              {userData.posts.map((post) => (
                <div className="user-post" key={post.pk}>
                  <Link href={`/posts/?postid=${post.pk}&page=1`}>
                    <a>
                      <strong>
                        {" "}
              ...
              {userData.posts.length == 5 && (
                ...
                >
                  <p className={styles.button}>view more</p>
                </Link>
              )}
            </div>
          </div>
          <div className={`col-md-6 ${styles.profileCol}`}>
            <div className={styles.colCard}>
              <Link
                href={
                  query.user
                    ? `/user/answers?user=${query.user}&page=1`
                    : `/user/answers?page=1`
                }
              >
                <a>
              ...
              {userData.solutions.map((solution) => (
                <div className="user-solution" key={solution.pk}>
                  <Link href={`/posts/?postid=${solution.post.pk}&page=1`}>
                    <a>
                      <strong>
                        {" "}
                        {solution.post.title.length > 30
                          ? solution.post.title.substring(0, 20) + "..."
                          : solution.post.title}
                      </strong>
                    </a>
                  </Link>
